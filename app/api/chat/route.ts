...
import { findDatabaseModelById } from "@/lib/database-model-config"
import {
...
async function handleChatRequest(req: Request): Promise<Response> {
    ...
    // PostgreSQL shared model (admin-configured): resolve credentials server-side only
    let databaseModelConfig: {
        provider: string
        modelId: string
        apiKey: string
        baseUrl: string | null
    } | null = null
    if (selectedModelId?.startsWith("db:")) {
        const uuid = selectedModelId.slice(3)
        const row = await findDatabaseModelById(uuid)
        console.log(
            `[DB Model Lookup] ID: ${selectedModelId}, Found: ${!!row}, Provider: ${row?.provider}`,
        )
        if (!row) {
            return Response.json(
                {
                    error: "Selected database model not found or disabled.",
                },
                { status: 404 },
            )
        }
    ...
    if (!databaseModelConfig && selectedModelId?.startsWith("server:")) {
    ...
    // DB models: ignore client headers for provider/model/api key/base URL (SSRF + secret safety)
    const clientOverrides = databaseModelConfig
        ? {
              provider: databaseModelConfig.provider,
              baseUrl: databaseModelConfig.baseUrl || undefined,
              apiKey: databaseModelConfig.apiKey,
              modelId: databaseModelConfig.modelId,
              awsAccessKeyId: null,
